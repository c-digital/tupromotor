...
{
    return CommissionHistory::where('order_id', $order)->sum('admin_commission') ?? null;
}

function count_sales($product)
{
    return App\Models\OrderDetail::where('product_id', $product)->sum('quantity');
}

function amount_sales($product)
{
    return App\Models\OrderDetail::where('product_id', $product)->sum('price');
}
