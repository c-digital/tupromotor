    ...
    <div class="card">
        ...
        <div class="card-body">
          <form action="{{ route('sellers.update', $seller->id) }}" method="POST">
                ...
                <div class="form-group row">
                    <label class="col-sm-3 col-from-label" for="commission">{{translate('Comisión')}}</label>
                    <div class="col-sm-9">
                        <input type="number" step="0.1" placeholder="{{translate('Comisión')}}" id="commission" name="commission" class="form-control" value="{{$seller->commission}}">
                    </div>
                </div>
