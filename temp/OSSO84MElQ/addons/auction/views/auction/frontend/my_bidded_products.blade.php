            ...
            <table class="table aiz-table mb-0">
                ...
                <tbody>
                    @foreach ($bids as $key => $bid_id)
                        @php
                            $bid = \App\AuctionProductBid::find($bid_id->id);;
                        @endphp
                        <tr>
                            ...
                            <td class="text-right">
                                @php
                                    $order = null;
                                    $order_detail = \App\OrderDetail::where('product_id',$bid->product_id)->first();
                                    if($order_detail != null ){
                                        $order = \App\Order::where('id',$order_detail->order_id)->where('user_id',Auth::user()->id)->first();
                                    }
                                @endphp
